...
> ⚠️ **중요 (에이전트 작업 시):** 배포되는 **플레이 링크(아티팩트)는 외부 이미지 파일을 로드하지 못합니다**(CSP).
> 즉 아래 `assets/sprites/*.png` 방식은 **GitHub Pages에서만** 보이고 플레이 링크에선 안 나와요.
> **모든 런타임 아트는 "코드 안의 SVG"로 작업하세요** → `AGENTS.md` §2, `docs/VISUAL_GUIDE.md`, `docs/ASSET_CHECKLIST.md` 참고.
> 이 문서(PNG 파이프라인)는 GitHub Pages 전용 보조 옵션입니다.
