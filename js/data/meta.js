...
const QUEST_TEMPLATES = [
  ...
  { id: "sell",   type: "sell",   nm: "자원 판매",   unit: "원", min: 600, max: 1500, moneyPer: 0.3 },
...
export function generateDailyQuests(dateStr) {
  ...
  const list = pool.map((t, i) => {
    const goal = t.type === "sell" ? rint(t.min, t.max) : rint(t.min, t.max);
    return {
      key: t.id + "_" + i, type: t.type, nm: t.nm, unit: t.unit, goal,
      progress: 0, done: false, claimed: false,
      rewardMoney: Math.round(goal * t.moneyPer),
      rewardStar: i === 0 ? 1 : 0, // 첫 퀘스트에 별머니 1
    };
  });
...
export const ACHIEVEMENTS = [
  ...
  { id: "rich",       nm: "부자",      pic: "💰", desc: "게임머니 10,000 보유", check: (s, S) => S.money >= 10000 },
...
export const STAR_SHOP = [
  { id: "elixir",   nm: "만능 물약",   pic: "✨", cost: 2, desc: "체력·피로 완전 회복", apply: (S) => { S.hp = S.maxHp; S.fatigue = 0; } },
  { id: "fastfarm", nm: "성장촉진제",  pic: "🌱", cost: 1, desc: "모든 농작물 즉시 완성", apply: (S) => { S.farm.forEach((p) => { const c = CROPS[p.cropId]; if (c) p.progress = c.grow; }); } },
  { id: "goldtool", nm: "황금 곡괭이", pic: "⛏️", cost: 4, desc: "도구 레벨 +1 (영구)", apply: (S) => { S.tool += 1; } },
  { id: "charm",    nm: "행운의 부적", pic: "🍀", cost: 5, desc: "채집 수확 +1 (영구)", apply: (S) => { S.mods.gatherBonus += 1; } },
  { id: "medal",    nm: "명예 훈장",   pic: "🎖️", cost: 3, desc: "공격력 +3 (영구)",   apply: (S) => { S.mods.atkBonus += 3; } },
];
